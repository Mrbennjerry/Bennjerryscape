...
public class KalphiteKingCombat extends CombatScript {
	...
	for (Entity e : npc.getPossibleTargets())
	{
	    ...
	    Projectile projectile = World.sendProjectile(npc, e, false, true, 0, 3747, 30, 30, 25, 2, 0, 0);
	    int delay = Utils.projectileTimeToCycles(projectile.getEndTime()) - 1;
	    Hit hit = getRangeHit(npc, getMaxHit(npc, 1460, NPCCombatDefinitions.RANGE, e));
	    hit.setAbilityMark();
	...
	for (Entity e : list)
	{
		...
		WorldTasksManager.schedule(new WorldTask() 
		{
		    ...
		    public void run() 
		    {
			e.getEffectsManager().startEffect(new Effect(EffectType.FRAGMENTATION, 10, HitLook.RANGE_DAMAGE, new Graphics(3574), Utils.random(1280)+1, 2, npc, new WorldTile(e)));
		    }
	...
	while (c < 3)
	{
		...
		Hit hit = new Hit(npc, 520 + Utils.random(2500), HitLook.RANGE_DAMAGE);
		hit.setAbilityMark();
		delayHit(npc, Utils.projectileTimeToCycles(projectile.getEndTime()) + 2, t, hit);
		t.getEffectsManager().startEffect(new Effect(EffectType.INCENDIARY_SHOT, 5));
	...
	if(target instanceof Player) 
	{
	    ...
	    ((Player)target).setNextColour(new Colour(projectile.getEndTime(), 580 - projectile.getEndTime(), 100, 40, 40, 100));
	    ...
	    ((Player)target).getEffectsManager().removeEffect(EffectType.DEVOTION);
	    ((Player)target).getEffectsManager().removeEffect(EffectType.BARRICADE);
	    ((Player)target).getEffectsManager().removeEffect(EffectType.IMMORTALITY);
	    ((Player)target).getEffectsManager().removeEffect(EffectType.RESONANCE);
	}
	WorldTasksManager.schedule(new WorldTask() {
	    @Override
	    public void run() 
	...
	for (Entity t : npc.getPossibleTargets())
	{
		...
		t.getEffectsManager().startEffect(new Effect(EffectType.STUNNED, 8));
	}
    }
	...
	{
	    if (t instanceof Player)	//Ugly but simple code for checking if player should be pushed back
	    {
	...
	if (t != null)
	{
	    ...
	    npc.setNextAnimation(new Animation(19449));
	    t.getEffectsManager().startEffect(new Effect(EffectType.SLAUGHTER, 10, HitLook.MELEE_DAMAGE, new Graphics(3464), Utils.random(1280)+1, 2, npc, new WorldTile(target)));
	}
	WorldTasksManager.schedule(new WorldTask()
	{
	    @Override
    ...
    private void rush(NPC npc, Entity target)	//Needs some improvement, if king starts the bullrush too close to the wall it won't do any damage to players, credits for 90% of this method goes to matrix developers
    {
	KalphiteKing king = (KalphiteKing) npc;
	king.setTarget(null);
    ...
    private void mageBall(NPC npc, Entity target, int n, boolean bleed)	//Single ball
    {
		...
		public void run() 
		{
			...
			public void run() 
			{
				...
				if (t.withinDistance(tile, 2)) 
				{
				    if(!bleed) 
				    {
					Hit hit = new Hit(npc, Utils.random(4350) + 100, HitLook.MAGIC_DAMAGE);
					hit.setAbilityMark();
					t.applyHit(hit);
				    }
				    else
	...
	for (Entity t : npc.getPossibleTargets()) 
	{
	    ...
	    int delay =  Utils.projectileTimeToCycles(projectile.getEndTime()) - 1;
	    Hit hit = getMagicHit(npc, getMaxHit(npc, Utils.random(2200), NPCCombatDefinitions.MAGE, t));
	    if (!(Utils.random(100) - ((Player)target).getSkills().getLevel(Skills.MAGIC) / 2 < 10) && hit.getDamage() > 1)
	    {
		t.setStunDelay(8);
		hit.setAbilityMark();
    ...
    {
	KalphiteKing king = (KalphiteKing) npc;
	((Player)target).getPackets().sendGameMessage("Phase " + king.getPhase());
	if ((king.isStunned() || king.isBound()) && (king.getId() == 16699 || king.getId() == 16698))	//Only on range and mage phase
	{
	    ...
	    if (king.getPhase() != 5 && king.getPhase() != 6) //Can't skip green after he started it
		king.nextPhase();
	    else if (king.getPhase() == 5)
		king.setPhase(7);
	    ((Player)target).getPackets().sendGameMessage("Was stunned! - Attack skipped");
	...
	if((king.getHPPercentage() < 75 && king.getSpawnCount() < 1) && ((king.getPhase() != 6 && king.getId() == 16699) || (king.getPhase() != 9 && king.getId() == 16697) || king.getId() == 16698))
	{
	    king.battleCry();
	    king.spawnCount++;
	...
	else if ((king.getHPPercentage() < 25 && king.getSpawnCount() < 2) && ((king.getPhase() != 6 && king.getId() == 16699) || (king.getPhase() != 9 && king.getId() == 16697) || king.getId() == 16698))
	{	//Same here
	    king.battleCry();
	    king.battleCry();
	    king.spawnCount++;
	...
	if (Utils.random(30) == 15 && king.getPhase() != 5 && king.getPhase() != 9)
	{
	    king.switchPhase();
	}
