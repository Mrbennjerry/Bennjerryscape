...
public final class Barrows extends Controller {
	...
	public void sendReward() {
		...
		int petRoll = Utils.random(250);
		if (petRoll == 1) {
			drop(PET_REWARDS[Utils.random(PET_REWARDS.length)]);
			player.getPackets().sendGameMessage("<col=AB0003>You recieved a barrows pet drop!");
		} else {
		    	player.getPackets().sendGameMessage("<col=AB0003>You rolled a "+petRoll+", but needed a 1 for a pet.");
		}
		for (int i = 0; i < 10; i++)
			if (percentage >= Math.random() * 100)
				drop(COMMUM_REWARDS[Utils.random(COMMUM_REWARDS.length)]);
		drop(new Item(995, Utils.random(50307))); // money reward at least always
