...
public class UnderGroundDungeon extends Controller {
	...
	public void sendInterfaces() {
		if (requiresLightSource) {
			...
			//player.getInterfaceManager().sendMinigameInterface(hasLight ? (LightSource.hasExplosiveSource(player) ? 98 : 97) : 96);
			//if (!hasLight)
	...
	public void process() {
		...
		if (requiresLightSource) {
			if (ticks % 4 == 0) {
				if (!LightSource.hasLightSource(player)) {
					if (!player.isLocked())
						player.applyHit(new Hit(player, Utils.random(1, 10), HitLook.REGULAR_DAMAGE));
				}
			}
		}
	}
