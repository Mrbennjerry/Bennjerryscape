...
public class Summoning extends Action {
    ...
    public static Familiar createFamiliar(Player player, Pouch pouch) {
        player.getPackets().sendGameMessage("Spawning Familiar.");
        try {
            return (Familiar) Class.forName("com.rs.game.npc.familiar.impl." + (NPCDefinitions.getNPCDefinitions(getNPCId(pouch.getRealPouchId()) + 1)).getName().replace(" ", "").replace("ï", "i").replace("-", "").replace("(", "").replace(")", "")).getConstructor(Player.class, Pouch.class, WorldTile.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
        } catch (Throwable e) {
            if (!Settings.HOSTED)
                e.printStackTrace();
            return null;
    ...
    public boolean process(Player player) {
        ...
        if (new Item(pouch.getRealPouchId()).getDefinitions().isStackable())
            amount *= Settings.getXpRate(player);
        for (int i = 0; i < ticks; i++) {
            ...
            if (pouch.getRealPouchId() == 12425) {
        	player.getInventory().addItem(new Item(pouch.getRealPouchId()).getId(), amount * 10);
            } else {
