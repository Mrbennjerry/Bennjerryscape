...
public class Magic {
	...
	public static final void handleSpellOnItem(Player player, int spellId, byte slot) {
		...
		case 47: // high alch
			...
			player.getInventory().addItemMoneyPouch(new Item(995, (int) (target.getDefinitions().getValue() * (highAlch ? 0.6D : 0.3D))));
	...
	public static final void handleSpellOnEntity(final Player player, int spellId, Entity target) {
		...
		case 107://stat spy npc
			...
			player.getPackets().sendIComponentText(522, 3, "Creature's Max Hit: " + npc.getMaxHit(npc.getAttackStyle()));
	...
	public static final void handleSpell(Player player, int spellId, int packetId) {
		...
		case 111: // moonclan teleport
			sendLunarTeleportSpell(player, 69, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1, EARTH_RUNE, 2);
			break;
		case 113: // ourania teleport
			sendLunarTeleportSpell(player, 71, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1, EARTH_RUNE, 6);
			break;
		case 116: // south falador teleport
			sendLunarTeleportSpell(player, 72, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1, AIR_RUNE, 2);
			break;
		case 117: // waterbirth teleport
			sendLunarTeleportSpell(player, 72, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1, WATER_RUNE, 1);
			break;
		case 121: // barbarian teleport
			sendLunarTeleportSpell(player, 75, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1, FIRE_RUNE, 3);
			break;
		case 123: // North Ardroudge teleport
			sendLunarTeleportSpell(player, 76, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 1, WATER_RUNE, 5);
			break;
		case 127: // Khazard teleport
			sendLunarTeleportSpell(player, 78, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 2, LAW_RUNE, 2, WATER_RUNE, 4);
			break;
		case 137: // Fishing guild teleport
			sendLunarTeleportSpell(player, 85, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 3, LAW_RUNE, 3, WATER_RUNE, 8);
			break;
		case 140: // Catherbay teleport
			sendLunarTeleportSpell(player, 87, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 3, LAW_RUNE, 3, WATER_RUNE, 10);
			break;
		case 143: // Ice Plateau teleport
			sendLunarTeleportSpell(player, 89, getSpellXP(data), getSpellTeleport(data), ASTRAL_RUNE, 3, LAW_RUNE, 3, WATER_RUNE, 8);
			break;
		...
		case 144:
		case 149:
			int index = spellId == 112 ? 0 : spellId == 118 ? 1 : spellId == 124 ? 2 : spellId == 128 ? 3 : spellId == 138 ? 4 : spellId == 142 ? 5 : spellId == 144 ? 6 : 7;
			if (!checkRunes(player, data, true))
				return;
			...
			for (int regionId : player.getMapRegionsIds()) {
				...
				for (Integer playerIndex : playersIndexes) {
					Player p2 = World.getPlayers().get(playerIndex);
					if (p2 == null || p2.isDead() || p2.hasFinished() || !p2.isRunning() || !p2.isAcceptingAid() && player.getIndex() != p2.getIndex() || p2.getInterfaceManager().containsScreenInterface() || !p2.withinDistance(player, 5))
						continue;
		...
		case 156: // crossbow bolt enchant
			...
			break;
		case 19: // mobi
			sendNormalTeleportSpell(player, 10, getSpellXP(data), getSpellTeleport(data), LAW_RUNE, 1, WATER_RUNE, 1, AIR_RUNE, 1);
			break;
		case 28: // varrock
			sendNormalTeleportSpell(player, 25, getSpellXP(data), getSpellTeleport(data), FIRE_RUNE, 1, AIR_RUNE, 3, LAW_RUNE, 1);
			break;
		case 31: // lumby
			sendNormalTeleportSpell(player, 31, getSpellXP(data), getSpellTeleport(data), EARTH_RUNE, 1, AIR_RUNE, 3, LAW_RUNE, 1);
			break;
		case 34: // fally
			sendNormalTeleportSpell(player, 37, getSpellXP(data), getSpellTeleport(data), WATER_RUNE, 1, AIR_RUNE, 3, LAW_RUNE, 1);
			break;
		case 36: // house
		...
		case 45: // ardy
			...
			break;
		case 50: // watch
			sendNormalTeleportSpell(player, 58, getSpellXP(data), new WorldTile(2549, 3114, 2), EARTH_RUNE, 2, LAW_RUNE, 2);
			break;
		case 57: // troll
