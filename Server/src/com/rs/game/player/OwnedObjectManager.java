...
public class OwnedObjectManager {
	...
	public static String addOwnedObjectManagerTemp(Player player, WorldObject[] object, int time) {
		return new OwnedObjectManager(player, object, time).managerKey;
	}
	...
	private OwnedObjectManager(Player player, WorldObject[] objects, int time) {
		managerKey = player.getUsername() + "_" + keyMaker.getAndIncrement();
		this.objects = objects;
		this.player = player;
		spawnObject(time);
	...
	private void spawnObject(int time) {
		World.spawnObjectTemporary(objects[count], time);
	}
