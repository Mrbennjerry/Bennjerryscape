...
public class CosmeticsManager implements Serializable {
	...
	private void sendPreviewOptions(int id, boolean hide) {
		...
		boolean canBuy = (isAutoUnlocked(id) || hasCosmeticVar(id, false));
		
		//player.getPackets().sendHideIComponent(1311, 174, hide || canBuy);
		player.getPackets().sendHideIComponent(1311, 194, false);
		if(!hide && canBuy)
			player.getPackets().sendIComponentText(1311, 681, "Apply");//unlockedCosmetics.contains(id) ? "Apply" : "Buy");
	}
			...
			for(int menuId = 0; menuId < MENUS_COMPONENT_IDS.length; menuId++) {
				if(MENUS_COMPONENT_IDS[menuId] == componentId) {
					...
					if(type == CosmeticType.WARDROBE) {
						...
						player.getPackets().sendGameMessage("dataId "+data.getId());
						...
						if(menuId != 20 && !(isAutoUnlocked(data.getId()) || hasCosmeticVar(data.getId(), data.getIntValue(2546) == 1)))
							player.getPackets().sendGameMessage("This cosmetic var is not currently added. Name: "+data.getValue(2533)+""+data.getId());
					}
					break;
				}
			}
	...
	private static void setVars() {
		...
		cosmeticsVars1.put(28417, 22649);//fire cape
		cosmeticsVars1.put(28211, 21633);
		cosmeticsVars1.put(28212, 21635);
		cosmeticsVars1.put(28213, 21637);
		cosmeticsVars1.put(28210, 22776);
		cosmeticsVars1.put(29808, 9579);
		cosmeticsVars1.put(29809, 9579);
		cosmeticsVars1.put(29810, 9579);
		cosmeticsVars1.put(29811, 9579);
