...
public class DivineHerblore extends Action {
	...
	public DivineHerblore(DivineHerbsSpots spot, int x, int y,int plane, Player owner) {
	    this.owner = owner;
		this.spot = spot;
	...
	private void addFish(Player player) {
		...
		if (player == owner) {
			...
			owner.getPackets().sendGameMessage(""+player.getUsername()+" picked some "+fish.getDefinitions().getName().toLowerCase()+" for you.");
		    }
		    player.getInventory().addItem(fish);
		    player.getPackets().sendGameMessage(getMessage(fish),  true);
		}
	...
	private boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.HERBLORE) < spot.getHerbs()[fishId].getLevel()) {
			player.getDialogueManager().startDialogue("SimpleMessage","You need a herblore level of " + spot.getHerbs()[fishId].getLevel() + " to fish here.");
			return false;
		}
