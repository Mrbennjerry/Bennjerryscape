...
public class DivineHunting extends Action {
	...
	public DivineHunting(DivineHuntingSpots spot, int x, int y,int plane, Player owner) {
	    this.owner = owner;
		this.spot = spot;
	...
	private void addFish(Player player) {
		...
		Item notedfish = new Item(spot.getHerbs()[fishId].getId()+1, multipleCatch ? 2
                        : 1);
		if (fish.getDefinitions().isStackable())
		    notedfish = fish;	
		...
		if (player == owner) {
			...
			owner.getPackets().sendGameMessage(""+player.getUsername()+" caught some "+fish.getDefinitions().getName().toLowerCase()+" for you.");
		    }
		    player.getInventory().addItem(fish);
		    player.getPackets().sendGameMessage(getMessage(fish),  true);
		}
