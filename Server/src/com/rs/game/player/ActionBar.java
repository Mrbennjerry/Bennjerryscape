...
public class ActionBar implements Serializable {
	...
	public void setupBar() {
		...
		int mainAbilityType =  type == Combat.MELEE_TYPE ? MELEE_ABILITY_SHORTCUT : type == Combat.RANGE_TYPE ? RANGED_ABILITY_SHORTCUT : type == Combat.MAGIC_TYPE ? MAGIC_ABILITY_SHORTCUT : -1;
		...
		if(mainAbilityType != -1) {
			for(Long abilityId : ClientScriptMap.getMap(CS_DATA_ID[mainAbilityType - 1]).getValues().keySet()) {
				...
				if(!hasRequiriments(abilityId.intValue(), getAbilityData(mainAbilityType, abilityId.intValue()), false, true)) 
					continue;

				shortcuts[currentBar][shortcutCount++] = mainAbilityType == MELEE_ABILITY_SHORTCUT ? new MeleeAbilityShortcut(abilityId.intValue()) :

					mainAbilityType == RANGED_ABILITY_SHORTCUT ? new RangeAbilityShortcut(abilityId.intValue()) : 
						mainAbilityType == MAGIC_ABILITY_SHORTCUT ? new MagicAbilityShortcut(abilityId.intValue()) : null;
						if(shortcutCount == 12)
							break;
			}
		}
		...
		if(secondAbilityType != -1 && shortcutCount < 12) {
			for(Long abilityId : ClientScriptMap.getMap(CS_DATA_ID[secondAbilityType - 1]).getValues().keySet()) {
				if(!hasRequiriments(abilityId.intValue(), getAbilityData(secondAbilityType, abilityId.intValue()), false, true)) 
					continue;

				shortcuts[currentBar][shortcutCount++] = secondAbilityType == STRENGTH_ABILITY_SHORTCUT ? new StrengthAbilityShortcut(abilityId.intValue()) : null;
				if(shortcutCount == 12)
					break;
			}
		}
		...
		if(packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
			...
			if(shortcut.getType() == MAGIC_ABILITY_SHORTCUT && shortcut.getId(player) == 8
				...
				if (!(player.getActionManager().getAction() instanceof PlayerCombatNew))
					return;
				Entity target = ((PlayerCombatNew) player.getActionManager().getAction()).getTarget();
				...
				for (Entity e : PlayerCombatNew.getMultiAttackTargets(player, target, 3, 9))
					PlayerCombatNew.delayHit(e, 0, PlayerCombatNew.getHit(player, e, true, Combat.MAGIC_TYPE, 1.0, 1.5 + (ticks / 5), false, true, false));
				...
				//here
				if (!queue) {
					Entity target = player.getCombatDefinitions().getCurrentTarget();//((PlayerCombatNew) player.getActionManager().getAction()).getTarget();
					if (target != null) {
						boolean canSiphon = !(target instanceof NPC);
						if (target instanceof Player) {
							Player playerTarget = (Player) target;
							canSiphon = playerTarget.getCombatDefinitions().getSpecialAttackPercentage() > player.getCombatDefinitions().getSpecialAttackPercentage();
						}
						if (!canSiphon) {
							player.getPackets().sendGameMessage("You cannot use this ability, as you have at least as much adrenaline as your target.");
							removeQueueShortcut();
							return;
						}
			...
			if (shortcut.getType() == HEAL_ABILITY_SHORTCUT && shortcut.getId(player) == 1) {
				if(!queue) {
					...
					if(player.getCombatDefinitions().isCombatStance()) {
						if(player.getVarsManager().getBitValue(REGENERATION_VARBIT_ID) == 1) {
							player.getPackets().sendGameMessage("Regenerate will no longer be activated when you have left combat.");
							removeQueueShortcut();
							return;
						}
					...
					if(player.isUnderCombat()) //cuz stance changes 1 tick later
						return;
					removeQueueShortcut();
				}
				//turn it on here
				...
				if (queue) {
					...
					if (!(player.getActionManager().getAction() instanceof PlayerCombatNew)) {
						...
						target = ((PlayerCombatNew) player.getActionManager().getAction()).getTarget();
					...
					if (target == null || !PlayerCombatNew.isWithinDistance(player, target) || player.isStunned())
						return;
				...
				else if ((shortcut.getType() == MELEE_ABILITY_SHORTCUT || shortcut.getType() == DEFENCE_ABILITY_SHORTCUT || shortcut.getType() == STRENGTH_ABILITY_SHORTCUT || shortcut.getType() == RANGED_ABILITY_SHORTCUT || shortcut.getType() == HEAL_ABILITY_SHORTCUT || shortcut.getType() == MAGIC_ABILITY_SHORTCUT) && hasCooldown(shortcut)) {
					if (player.getCombatDefinitions().isAllowAbilityQueueing()) {
						setQueueShortcut(shortcut, shortcutIndex);
						return;
					}
		...
		switch(shortcut.getType()) {
			case ITEM_SHORTCUT:
				...
				if(defs.isWearItem() && packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
				    player.getPackets().sendGameMessage("item"+packetId+" / "+WorldPacketsDecoder.ACTION_BUTTON1_PACKET);
					Item item = player.getEquipment().getItem(defs.getEquipSlot());
					if (item != null && item.getId() == shortcut.getId(player)) {
						player.getEquipment().handleEquipment(defs.getEquipSlot(), shortcut.getId(player), WorldPacketsDecoder.ACTION_BUTTON1_PACKET);
						return;
					}
				}
	...
	private static final int[][] SHARED_CD_MAPS = {
		{14682, 14699, 14664, 14664},//Binding abilities
	...
	private boolean hasRequiriments(int abilityId, GeneralRequirementMap data, boolean warn, boolean setup) {
		...
		if (abilityStyle > 0) {
			...
			if(!setup && abilityType != 0 && player.getCombatDefinitions().getSpecialAttackPercentage() < (abilityType == THRESHOLD_ABILITY ? 50 : 100)) {
				if(warn)
					player.getPackets().sendGameMessage((abilityType == THRESHOLD_ABILITY ? "Threeshold" : "Ultimate")+ " abilities require you to have "+(abilityType == THRESHOLD_ABILITY ? 50 : 100)+"% adrenaline before they can be used.");	
				return false;
			}
		}
		return true;
	...
	private static boolean hasAbilityStyle(Player player, int abilityStyle, boolean mainHand) {
		...
		return ((abilityStyle == 1 ||
				abilityStyle == 2) && handStyle == Combat.MELEE_TYPE)
	...
	private void triggerAbility(Shortcut shortcut) {
		...
		if(bookId == HEAL_ABILITY_SHORTCUT && abilityId == 16) { //special attack exeption
			Item weapon = player.getEquipment().getItem(Equipment.SLOT_WEAPON);
			if (weapon == null)
				return;
		...
		if(!shortcut.queue(player)) { //code one click abilities here
				...
				case MAGIC_ABILITY_SHORTCUT:
					switch(abilityId) {
						case 2://SURGE
							...
							player.setNextForceMovement(new NewForceMovement(player, 0, tile, 1, Utils.getAngle(tile.getX() - player.getX(), tile.getY() - player.getY())));
							...
							break;
						case 8://DETONATE
							if(!(player.getActionManager().getAction() instanceof PlayerCombatNew) || player.getEffectsManager().hasActiveEffect(EffectType.DETONATE))  {
								//message rs says when no target
								return;
							}
							//turn detonate on. make it do emote(check how emote is being gathered at combat)
							player.getEffectsManager().startEffect(new Effect(EffectType.DETONATE, 10, 0));// Six seconds
							break;
						case 10://METAMORPHISIS
							GeneralRequirementMap spellData = Magic.getSpellData(player.getCombatDefinitions().getSpellId());
							player.getEffectsManager().startEffect(new Effect(EffectType.METAMORPHISIS, 35, Magic.getSpellType(spellData)));
							break;
					}
					break;
				case RANGED_ABILITY_SHORTCUT:
					switch(abilityId) {
						case 3://ESCAPE
							...
							player.setNextForceMovement(new NewForceMovement(player, 0, tile, 1, Utils.getAngle(player.getX() - tile.getX(), player.getY() - tile.getY())));
							WorldTasksManager.schedule(new WorldTask() {
					...
					switch(abilityId) {
						case 2://BARGE
							...
							if(target != null) {
								...
								if (player.getActionManager().getAction() instanceof PlayerCombatNew)
									combat = (PlayerCombatNew) player.getActionManager().getAction();
								else {
								...
								combat.setNextAbility(new StrengthAbilityShortcut(200));// Kick is the next ability overide
							}
							player.setAttackingDelay(Utils.currentTimeMillis() + 4000);
							player.lock(2);
							player.setNextAnimation(new Animation(18147));
							player.setNextGraphics(new Graphics(3580));
							player.setNextForceMovement(new NewForceMovement(player, 0, tile, 1, player.getDirection()));
				...
				case DEFENCE_ABILITY_SHORTCUT:
					...
					switch(abilityId) {
						...
						case 10:
							...
							break;
						case 11://REJUVINATE
							effect = new Effect(EffectType.REJUVINATE, 17, HitLook.HEALED_DAMAGE, null, (int) ((player.getMaxHitpoints() * 0.4) * 0.05), 2, player);
				...
				case HEAL_ABILITY_SHORTCUT:
					switch(abilityId) {
						...
						case 9://GUTHIX_BLESSING
							player.getEffectsManager().startEffect(new Effect(EffectType.GUTHIX_BLESSING, 17, new GuthixBlessing(player, new WorldTile(player))));
							break;
						case 14://ICE ASYLUM
							player.getEffectsManager().startEffect(new Effect(EffectType.ICE_ASYLUM, 33, new WorldTile(player), player.getMaxHitpoints() * 3));
							break;
			...
			PlayerCombatNew combat = (PlayerCombatNew) player.getActionManager().getAction();
					...
					switch(abilityId) {
						...
						case 12://METEOR STRIKE
							...
							if (currentHP <= maxHP / 2) {
								player.getPackets().sendGameMessage("This ability may only be used on targets with over 50% life points remaining.");
								return;
							}
							break;
					}
		...
		if(!(abilityType == 0 && player.getCombatDefinitions().getCurrentTarget() == null))
			player.getCombatDefinitions().increaseSpecialAttack(abilityType == 0 ? player.getEffectsManager().hasActiveEffect(EffectType.NATURAL_INSTINCT) ? 16 : 8 : abilityType == 1 ? -15 : -100);
		...
		if (bookId == DEFENCE_ABILITY_SHORTCUT && abilityId == 14)//Manditory exception
			player.getEffectsManager().processBuffTimer(player.getEffectsManager().getEffectForType(EffectType.DEVOTION), true);
	}
...
var1: 4164, 4
	...
	private void setQueueShortcut(Shortcut shortcut, int shortcutIndex) {
		if(shortcutIndex != -1 && getQueueShortcutIndex() == shortcutIndex) { //-1 is outside book
			removeQueueShortcut();
			return;
		}
		...
		else if(queueShortcut != null && queueShortcut.getType() == HEAL_ABILITY_SHORTCUT && queueShortcut.getId(player) == 1) 
	...
	private void removeQueueShortcut() {
		...
		if(queueShortcut != null && queueShortcut.getType() == HEAL_ABILITY_SHORTCUT && queueShortcut.getId(player) == 1) 
			player.getVarsManager().sendVarBit(REGENERATION_VARBIT_ID, 0);
		queueShortcut = null;
	}
	...
	private static boolean isMagicAbility(int abilityId) {
		//abilty ids here to distinguish from normal spells
		return (abilityId == 7 || abilityId == 1 || abilityId == 12 || abilityId == 6
				|| abilityId == 165 || abilityId == 3 || abilityId == 166 || abilityId == 171 || abilityId == 5
				|| abilityId == 2 || abilityId == 8 || abilityId == 4 || abilityId == 9 || abilityId == 10 || abilityId == 11
	...
	public void processQueuedShortcut() {
		...
		/*	long currentTime = Utils.currentWorldCycle();
			...
			for(int i = 0; i < 9; i++) {
				...
				if(shortcut != null && (shortcut.getType() == MELEE_ABILITY_SHORTCUT || shortcut.getType() == DEFENCE_ABILITY_SHORTCUT
						...
						|| (shortcut.getType() == MAGIC_ABILITY_SHORTCUT && isMagicAbility(shortcut.getId(player)))) && !hasCooldown(shortcut)) {


					GeneralRequirementMap data = GeneralRequirementMap.getMap(ClientScriptMap.getMap(CS_DATA_ID[shortcut.getType()-1]).getIntValue(shortcut.getId(player)));
					int abilityType = data.getIntValue(2799);
	...
	private void pushShortcutOnSomething(Shortcut shortcut, Object target) {
		...
		switch(shortcut.getType()) {//todo dfs
			case MELEE_ABILITY_SHORTCUT:
			case DEFENCE_ABILITY_SHORTCUT:
			case RANGED_ABILITY_SHORTCUT:
			case HEAL_ABILITY_SHORTCUT:
	...
	public void sendShortcutVar(int barIndex, int type, int id, boolean force) {
		if(force) { //to save data from being sent every click lol
			player.getVarsManager().forceSendVar(barIndex >= 12 ? 4427 + (barIndex-12) : 811+barIndex, type != 0 ? -1 : id);
			player.getVarsManager().forceSendVar(barIndex >= 12 ? 4413 + (barIndex-12) : 727+barIndex, type != 0 ? (id << 4 | type) : 0);
		}else{
			player.getVarsManager().sendVar(barIndex >= 12 ? 4427 + (barIndex-12) : 811+barIndex, type != 0 ? -1 : id);
			player.getVarsManager().sendVar(barIndex >= 12 ? 4413 + (barIndex-12) : 727+barIndex, type != 0 ? (id << 4 | type) : 0);
		}
	}
	...
	private void unlockShortcuts(boolean menu) {
		for(int i = 0; i < shortcuts[currentBar].length; i++) {
			player.getPackets().sendIComponentSettings(menu ? 1436 : 1430,(menu ? 25 : 55) + i*13, -1, 1, !lockedBar || menu ? 15302654 : 2195454);
		}
	}

	public void refreshButtons() {
		player.getPackets().sendExecuteScript(6992); //unlocks action bar buttons properly
	}
	...
	private static boolean canSurge(Player player) {
		...
		boolean surgeDisabled = c != null && 
				(c instanceof BorkController || c instanceof DuelArena || c instanceof DTControler ||
						c instanceof FightCaves || c instanceof FightKiln || c instanceof HouseControler|| 
	...
	private static WorldTile getSurgeTile(Player player, int start, int end, boolean increment, Entity target) {
		...
		for (int steps = start; increment ? steps < end : steps > end; steps += (increment ? 1 : -1)) {
			...
			if (target != null && Utils.colides(target.getX(), target.getY(), target.getSize(), step.getX(), step.getY(), player.getSize()) || !player.clipedProjectile(step, true) || !World.isTileFree(step.getPlane(), step.getX(), step.getY(), player.getSize()))
				break;
			lastStep = step;
		}
		return lastStep;
