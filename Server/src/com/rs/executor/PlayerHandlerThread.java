...
public final class PlayerHandlerThread extends Thread {
	...
	private final void processPlayerSave(SaveDetails details) {
		OutputStream[] parts = LoginChannelsPacketEncoder.encodePlayerFileTransmit(details.username, details.data);
		LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodePlayerFileTransmitInit(details.username, details.data.length).getBuffer());
	...
	private final boolean processLogoutPlayer(Player player) {
		...
		if (data != null && data.length > 0) {
			OutputStream[] parts = LoginChannelsPacketEncoder.encodePlayerFileTransmit(player.getUsername(), data);
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodePlayerFileTransmitInit(player.getUsername(), data.length).getBuffer());
			for (int i = 0; i < parts.length; i++)
				LoginClientChannelManager.sendReliablePacket(parts[i].getBuffer());
		}
		LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodeLogout(player.getUsername()).getBuffer());
		return true;
	}
		...
		if (details.step == 0) { // send request to login server
			...
			if (details.response_status == 255) { // we need to wait bit more and then retry
				...
				if (details.response_filelength <= 0) { // new player
					...
					details.waitTime = Utils.currentTimeMillis() + Settings.LOGIN_SERVER_FILE_TIMEOUT;
					details.step = 4;
				}
			} else { // generic error reply
				details.session.getLoginPackets().sendClosingPacket(details.response_status);
				return true;
			}
	 ...
	 */
	public static final void handleLoginResponse(int sessionid, String sessionuser, int status, int file_length, int rights, boolean masterLogin, boolean donator, boolean extremeDonator, boolean support, boolean gfxDesigner, int messageIcon, boolean muted, long lastVote, String displayName, String email) {
		synchronized (lock) {
			LoginDetails details = waitingLoginSessions.get(sessionid);
			if (details == null && status == 2) {
				// we did not actually log in
				LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodeLogout(sessionuser).getBuffer());
	...
	public static final void handleLoginFileResponse(int sessionid, byte[] data) {
		synchronized (lock) {
			...
			int amt_write = Math.min(details.response_filelength - details.response_receivedamount, data.length);
			System.arraycopy(data, 0, details.response_filebuffer, details.response_receivedamount, amt_write);
	 ...
	 */
	public static final void addSession(Session session, int[] sessionKeys, boolean lobby, String username, String password, String MACAddress, int displayMode, int screenWidth, int screenHeight, MachineInformation machineInfo) {
		LoginDetails details = new LoginDetails(session, sessionKeys, lobby, username, password, MACAddress, displayMode, screenWidth, screenHeight, machineInfo);
		synchronized (lock) {
	...
	private static class LoginDetails {
		...
		public LoginDetails(Session session, int[] sessionKeys, boolean lobby, String username, String password, String MACAddress, int displayMode, int screenWidth, int screenHeight, MachineInformation machineInfo) {
			this.step = 0;
